...
from audio_safety.data.families import score_transcript_records
...
def test_word_error_rate_exact_and_substitution():
    ...
    assert word_error_rate("how to report malware safely", "how to discuss malware safely") == pytest.approx(0.2)


def test_token_overlap_content_words():
    assert token_overlap("How can I report malware safely?", "report malware safely") == pytest.approx(1.0)
