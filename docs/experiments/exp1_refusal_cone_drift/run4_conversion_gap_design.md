...
- **행동:** family별 flip rate, benign DiD specificity, modality·safety-label에 blind인 OpenRouter
  **Gemini Flash-Lite judge 1개**(`google/gemini-3.5-flash-lite`; 2026-07-13 비용 절감으로
  flash→flash-lite, 아래 변경 이력) + **genuine-refusal vs
...
- 2026-07-13 — **§8 구현 착수 + judge 비용 절감.** attack-induced-flip 파이프라인 최소 구현
  (frozen jailbreak 템플릿 `configs/attacks/jailbreak_templates.yaml` → `prepare_attack_variants.py`
  → `style` 축 재사용 렌더/행동/text-leg/judge → `evaluation/attack_flip.py`/`analyze_flip.py`,
  config `configs/experiments/run4_attack_flip.yaml`). 사용자 결정으로 direction-finding judge를
  `google/gemini-3.5-flash` → **`google/gemini-3.5-flash-lite`**(더 저렴한 tier)로 교체. §0 표·§1
  가설·`design.md` §0 불변. Codex(gpt-5.6-sol xhigh) blind 코드 감사 2라운드로 통계 3종
  (flip/DiD/interaction) 독립 정확성 확인. 반영한 무결성 수정: (a) text-attack style에 hash-verified
  wrapper 없으면 렌더 fail-closed(누락→clean 텍스트 오분류 차단, emotion 파일 우회 불가), (b) frozen
  attack 파일 `rendered_sha256` 로드시 검증 + 중복키 거부 + spoken(strip) 텍스트로 해시 정규화,
  (c) 각 wav에 `<wav>.sha256` sidecar 기록 + 재사용시 텍스트 변경 감지(불일치→raise), attack run은
  격리된 `audio_attack_flip/` 사용, (d) `text_modality`를 결과에 명시 라벨.
  **이 run의 스코프(정직성):** 이번 direction-finding은 **primary jailbreak family만** 사용하고
  (emotion 2차 family는 config에 주석으로 이연), 템플릿은 공개 벤치마크 verbatim이 아니라
  **ICA/PAP 계열을 재현한 authored wrapper를 hash-freeze**한 것이다. 결과를 "완결된 2-family §8 증거"나
  "특정 공개 ICA/PAP 벤치마크 성능"으로 제시하지 않는다(paper-facing 확정에서 재설계).
