"""Shared CLI for the post-prefill KV-routing experiments (Exp4 and Exp5).

Both experiments run the same pipeline; they differ only in their frozen
condition set, config file and run-name prefix.
"""
...
def _parse_args(prefix: str, default_config: Path, description: str) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--run-name",
        required=True,
        help=f"{prefix}_YYYYMMDD_HHMM_tag; reuse for resumable stage invocations",
    )
...
def run_cli(*, prefix: str, default_config: Path, description: str) -> None:
    args = _parse_args(prefix, default_config, description)
    if re.fullmatch(rf"{prefix}_\d{{8}}_\d{{4}}_[a-z0-9_]+", args.run_name) is None:
        raise ValueError(f"--run-name must match {prefix}_YYYYMMDD_HHMM_tag")
